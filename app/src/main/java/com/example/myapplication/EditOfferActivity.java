...
public class EditOfferActivity extends AppCompatActivity {
    ...
    private void updateRoomDetails() {
        ...
        DatabaseReference databaseReference = FirebaseDatabase.getInstance().getReference("Offers").child(roomName);
        ...
        databaseReference.updateChildren(updatedValues).addOnCompleteListener(task -> {
            if (task.isSuccessful()) {
                Toast.makeText(EditOfferActivity.this, "Room details updated successfully!", Toast.LENGTH_SHORT).show();
                clearFields();
            } else {
                Toast.makeText(EditOfferActivity.this, "Failed to update: " + task.getException(), Toast.LENGTH_SHORT).show();
            }
        });
    }
